...
//printf("%d\n",pivot_i);
	...
	//printf("finished left %d right %d\n",init_left, init_right);

	int save_threads_used = -1;
	if(right-1-init_left>=1)
	{
		...
		{	//give this part of the array to another thread
			...
			save_threads_used = threads_used;
	...
	if(save_threads_used > -1) {
		//printf("Thread %d waits for its children...\n", id);
		pthread_join(threads[save_threads_used ],NULL);
	}
}
...
//	printf("NB_THREADS = %d\n",NB_THREADS);
	...
	//printf("Wait end thread\n");

 	// wait all threads
	for(int i=0;i<1 /*NB_THREADS*/;++i)
	{
