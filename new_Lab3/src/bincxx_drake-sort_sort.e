...
void mergeSort(int* array, int size)
{
 ...
 for(int i=0;i<0;i++)
 {
  ...
  pthread_create(&threads[i], __null, &split, (void*)&args[i]);
 }

 for(int i=0;i<0;i++)
 {
  pthread_join(threads[i], __null);
 }
 ...
 if(0<4)
 {
  ...
  for(int i=0;i<0;++i) idx[i] = args[i].left;
  for(int i=0;i<size;++i)
  {
   int min_i = -1;
   int min = 2147483647;
 ...
 }else if(0 == 4)
 {
  ...
  pthread_create(&threads[0], __null, &p_merge, (void*)&args[0]);
  ...
  pthread_create(&threads[1], __null, &p_merge, (void*)&args[1]);



  pthread_join(threads[0], __null);
  pthread_join(threads[1], __null);
